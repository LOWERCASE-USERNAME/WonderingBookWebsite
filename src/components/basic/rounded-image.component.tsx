...
export function RoundedImage({ src, id, className, isReadOnly = true, setCardImage, onUpdate }: RoundedImageProps) {
  ...
  if (src) {
    return (
      <img className={cn(" rounded-full w-40 h-40 object-cover border-2 border-black", className)} src={src} />
    );
  }
  ...
  if (!isReadOnly) {
    ...
    return (
      <label
        className={cn("rounded-full w-40 h-40 object-cover border-2 border-black", className)} htmlFor="file"
        onClick={handleChooseImage}
      >
        <div className="flex flex-col items-center justify-center h-full">
          <svg width="36px" xmlns="http://www.w3.org/2000/svg" fill="" viewBox="0 0 24 24">
          ...
          <span className="text-sm">Thêm hình ảnh mới</span>
        </div>
